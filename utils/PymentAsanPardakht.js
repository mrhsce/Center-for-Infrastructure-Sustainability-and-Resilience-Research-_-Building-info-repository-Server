...
module.exports = {
  _encrypt: (text) => {
    var key = CryptoJS.enc.Utf8.parse(
      Buffer.from(asanPardakht.EncryptionKey, "base64").toString("utf8", 0, 32)
    );
    var iv = CryptoJS.enc.Utf8.parse(
      Buffer.from(asanPardakht.EncryptionVector, "base64").toString(
        "utf8",
        0,
        16
      )
  ...
  encrypt: (text) => {
    ...
    var cipher = crypto.createCipheriv(
      ...
      ivBuf.toString("utf8").slice(0, 16)
    );
  ...
  decrypt: (text) => {
    var decipher = crypto.createDecipheriv(
      algorithm,
      asanPardakht.EncryptionKey,
      asanPardakht.EncryptionVector
    );
  ...
  createRequest: (
    ...
    return (
      codyType +
      "," + // P1
      asanPardakht.USERNAME +
      "," + // P2
      asanPardakht.PASSWORD +
      "," + // P3
      invoiceId +
      "," + // P4
      price +
  ...
  requestOperation: (encryptedRequest, callBack) => {
    const args = {
      merchantConfigurationID: asanPardakht.MERCHANT_CONFIG_ID,
